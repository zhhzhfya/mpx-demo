 ...
 * The output type of the file generated by this class depends on the file name
 * extension supplied by the user. A .xml extension will generate an MSPDI file,
 * anything else will generate an MPX file.
 */
public class MpxjCreate {
	...
	/**
	 * This method creates a summary task, two sub-tasks and a milestone, all
	 * with the appropriate constraints between them. The tasks are assigned to
	 * two resources. Note that Microsoft Project is fussy about the order in
	 * which things appear in the file. If you are going to assign resources to
	 * tasks, the resources must appear in the file before the tasks.
	 *
	...
	private static void create(String filename) throws Exception {
		...
		//
		// Retrieve the project properties and set the start date. Note
		// Microsoft
		// Project appears to reset all task dates relative to this date, so
		// this
		...
		// the remaining work attribute. The assignment start dates will
		// normally
